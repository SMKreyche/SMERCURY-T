...
c Calculate obliquity (use Earth or body #2 for ghost planets)
...
c Local
      ...
      real*8 ci,to,temp,tmp2,bige,f,cf,ce,a
c
c------------------------------------------------------------------------------
c
      hx = y * w  -  z * v
...
c Longitude of perihelion
        cf = (s - r) / (e*r)
        if (abs(cf).gt.1.d0) cf = sign(1.d0,cf)
        f = acos(cf)
...
c
      if (l.lt.0.d0.and.e.lt.1.d0) l = l + TWOPI
      if (l.gt.TWOPI.and.e.lt.1.d0) l = mod (l, TWOPI)
c
c------------------------------------------------------------------------------
c
      return
