...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      SMERCURY-T.FOR (Steven M. Kreyche 2020)
c      - Based on SMERCURY.FOR and includes the option of GR forces as well as 
c        tidal effects on obliquity and rotation
c
c      SMERCURY.FOR    (ErikSoft   4 May 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
...
c This version of mercury follows the spin evolution of a planet treating it
c as a rigid body, following Touma and Wisdom (1994) Astron J. 107, 1189.
c
c------------------------------------------------------------------------------
...
c Variables:
c ---------
c  M      = mass (input as Solar masses, code then multiplies K2 factor for use in computations)
...
c  MOI    = principal moments of inertia (solar masses AU^2), now local in mdt_spin.for -- SMK
...
c          9  ->  apparently the spin-tracking one -- JWB
...
c  OPT(6) = apply obliquity tides (phase lag model)? (0=no, 1=yes) -- SMK
c  OPT(7) = apply obliquity tides (time lag model)? (0=no, 1=yes) -- SMK
c  OPT(8) = apply user-defined force routine mfo_user? (this routine includes general
c           relativistic accelerations for the spin algor = 9) (0=no, 1=yes) -- SMK
c
c File variables :
c --------------
...
c
      ...
      real*8 tcount(3,NMAX)
      character*8 id(NMAX)
      character*80 outfile(3), dumpfile(4), mem(NMESS)
      external mdt_mvs, mdt_bs1, mdt_bs2, mdt_ra15, mdt_hy, mdt_spin
...
c Main integration
     ...
     %  lmem,mdt_mvs,mco_h2mvs,mco_mvs2h,C_MR2,k_2p,t_lag,ttol,tcount)
c
      if (algor.eq.2) call mal_hvar (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_bs1,C_MR2,k_2p,t_lag,ttol,tcount)
c
      if (algor.eq.3) call mal_hvar (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_bs2,C_MR2,k_2p,t_lag,ttol,tcount)
c
      if (algor.eq.4) call mal_hvar (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_ra15,C_MR2,k_2p,t_lag,ttol,tcount)
c
      if (algor.eq.9) call mal_hcon (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_spin,mco_h2dh,mco_dh2h,C_MR2,k_2p,t_lag,ttol,tcount)
c     
      if (algor.eq.10) call mal_hcon (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_hy,mco_h2dh,mco_dh2h,C_MR2,k_2p,t_lag,ttol,tcount)
c
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MDT_SPIN.FOR    (ErikSoft   28 June 2003)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c
     ...
     %  outfile,mem,lmem,C_MR2,rho,k_2p,t_lag,ttol,tcount)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 k_2p(nbod),t_lag(nbod),ttol,tcount(3,nbod)
      character*80 outfile(3),mem(NMESS)
      character*8 id(nbod)
c
c Local
      integer j,nce,ice(NMAX),jce(NMAX),ce(NMAX),iflag,i,tflag(3,NMAX)
      integer check
      real*8 a(3,NMAX),hby2,x0(3,NMAX),v0(3,NMAX),mvsum(3),temp
      real*8 as(3,NMAX),bigs(3,NMAX),smag,c(3,3,NMAX)
      real*8 eqR(NMAX),moi(3,NMAX),N_t(3,NMAX),N_sum(3,NMAX)
      real*8 xh(3,NMAX),vh(3,NMAX),vh_GR(3,NMAX),v_GR(3,NMAX)
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
      save a,as,c,bigs,eqR,moi,tflag,N_sum,v_GR
c
      hby2 = h0 * 0.5d0
      nclo = 0
      colflag = 0
...
c Do calculations if tidal flag parameter requires them -- SMK
        if (tflag(1,i).eq.1.or.tflag(2,i).eq.1.or.tflag(3,i).eq.1) then
          check = 1
...
c Also calculate moi components and radii for body 2 and smalls -- SMK
     ...
     %                  moi(2,i),moi(3,i))
c
c If using obliquity tides (time lag) calculate body spins if necessary -- SMK
          if (opt(7).eq.1.and.dtflag.eq.2) then
c
c Convert spins to body frame using S = C^-1 * s
            bigs(1,i) = c(1,1,i)*s(1,i)+c(2,1,i)*s(2,i)+c(3,1,i)*s(3,i)
            bigs(2,i) = c(1,2,i)*s(1,i)+c(2,2,i)*s(2,i)+c(3,2,i)*s(3,i)
            bigs(3,i) = c(1,3,i)*s(1,i)+c(2,3,i)*s(2,i)+c(3,3,i)*s(3,i)
c
          end if
        end if
91      end do
c
c If using obliquity tides (time lag) calculate accelerations if tidal torque was added in previous time step -- SMK
        if (opt(7).eq.1.and.dtflag.eq.2.and.check.eq.1) then
          call mfo_spinorb (jcen,nbod,nbig,m,x,rcrit,a,as,stat,c,moi)
        end if
c
      end if      
c
c        WRITE (*,*) 'C matrix at time=',time/365.25d0
c        WRITE (*,111) c(1,1),c(1,2),c(1,3)
c        WRITE (*,111) c(2,1),c(2,2),c(2,3)
c        WRITE (*,111) c(3,1),c(3,2),c(3,3)
c 111    FORMAT (' (',f9.6,1x,f9.6,1x,f9.6,')')
c
...
c		  write (*,*) 'calling spinorb'
c		  write (*,*) ' x[1,10] = ', x(1,10), ' x[2,10] = ', x(2,10)  
c		  write (*,*) ' x[1,2]  = ', x(1,2), ' x[2,2]  = ', x(2,2) 
...
c Converting to heliocentric (central) coordinates for this part
          call mco_dh2h (time,jcen,nbig,nbig,h0,m,x,v,xh,vh,ngf,
     %                     ngflag,opt)
c
c Calculate relativistic accelerations
          call mfo_user (nbod,nbig,hby2,m,xh,vh,vh_GR)
c
c Convert velocities back to democratic heliocentric coordinates
          call mco_h2dh (time,jcen,nbig,nbig,h0,m,xh,vh_GR,x,v_GR,ngf,
     %                     ngflag,opt)
        end if
c
        dtflag = 2
...
c Advance interaction Hamiltonian for H/2
      do j = 2, nbig
        v(1,j) = v(1,j) + hby2 * a(1,j) + v_GR(1,j)
        v(2,j) = v(2,j) + hby2 * a(2,j) + v_GR(2,j)
        v(3,j) = v(3,j) + hby2 * a(3,j) + v_GR(3,j)
      end do
...
c     %    as(2,2)*hby2,as(3,2)*hby2
c        WRITE (*,222) 'After spinorb1, big S: ',bigs(1,2),bigs(2,2),
c     %    bigs(3,2),sqrt(bigs(1,2)*bigs(1,2)+bigs(2,2)*bigs(2,2)
c     %    +bigs(3,2)*bigs(3,2))
c
...
c
c Advance the Euler Hamiltonian
      call mfo_spin (nbod,bigs,c,h0,moi,nbig)
c        WRITE (*,222) 'After Euler,    big S: ',bigs(1,2),bigs(2,2),
c     %    bigs(3,2),sqrt(bigs(1,2)*bigs(1,2)+bigs(2,2)*bigs(2,2)
c     %    +bigs(3,2)*bigs(3,2))

c	set ghost planets to planet 2 location
      do j = nbig+1, nbod
        x(1,j) = x(1,2)  
        x(2,j) = x(2,2)
        x(3,j) = x(3,2)
...
c      call mce_snif (h0,2,nbod,nbig,x0,v0,x,v,rcrit,ce,nce,ice,jce)
c
...
c     %    jxvclo,outfile,mem,lmem,mfo_hkce)
c      end if
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
94    end do
c
c Convert spins to inertial frame
      do j = 2, nbod
c			if (j.eq.2) then
c				write(*,*) 'spinb4 for planet ', j, ': s(1)=', s(1,j)
c				write(*,*) '      s(2)=', s(2,j), ', and s(3)=', s(3,j)
c				write(*,*) '      s mag=', sqrt(s(1,j)*s(1,j)+s(2,j)*s(2,j)
c     %	      +s(3,j)*s(3,j))
c	  		endif
c			if (j.eq.10) then
c				write(*,*) 'spinb4 for planet ', j, ': s(1)=', s(1,j)
...
c	  		endif
        ...
        s(1,j) =c(1,1,j)*bigs(1,j)+c(1,2,j)*bigs(2,j)+c(1,3,j)*bigs(3,j)
        s(2,j) =c(2,1,j)*bigs(1,j)+c(2,2,j)*bigs(2,j)+c(2,3,j)*bigs(3,j)
        s(3,j) =c(3,1,j)*bigs(1,j)+c(3,2,j)*bigs(2,j)+c(3,3,j)*bigs(3,j)
		  
c			if (j.eq.2) then
c				write(*,*) 'spinafter for planet ', j, ': s(1)=', s(1,j)
c				write(*,*) '      s(2)=', s(2,j), ', and s(3)=', s(3,j)
c				write(*,*) '      s mag=', sqrt(s(1,j)*s(1,j)+s(2,j)*s(2,j)
c     %	      +s(3,j)*s(3,j))
c	  		endif
...
c        WRITE (*,*) 'as*h:                  ',as(1,2)*hby2,
c     %    as(2,2)*hby2,as(3,2)*hby2
c        WRITE (*,222) 'After spinorb2, big S: ',bigs(1,2),bigs(2,2),
c     %    bigs(3,2),sqrt(bigs(1,2)*bigs(1,2)+bigs(2,2)*bigs(2,2)
c     %    +bigs(3,2)*bigs(3,2))
c
...
c (only need body 2 coords)
        call mco_dh2h (time,jcen,2,nbig,h0,m,x,v,xh,vh,ngf,
     %                 ngflag,opt)
c
c Computing tidal obliquity torque components
        call obl_tides(nbod,nbig,m,xh(1,2),xh(2,2),xh(3,2),vh(1,2),
     %                 vh(2,2),vh(3,2),s,C_MR2,eqR,k_2p,t_lag,N_t,h0)
c
...
c If flagged, zero out torque sums
          if (tflag(1,j).eq.1) N_sum(1,j) = 0.d0
          if (tflag(2,j).eq.1) N_sum(2,j) = 0.d0
          if (tflag(3,j).eq.1) N_sum(3,j) = 0.d0
c
...
c Add torque sums to spin vectors if magnitude exceeds tidal tolerance threshold
          if (abs(N_sum(1,j)).ge.abs(s(1,j)*ttol)) then
            s(1,j) = s(1,j) + N_sum(1,j)
            tflag(1,j) = 1
            tcount(1,j) = tcount(1,j) + 1.d0
          else 
            tflag(1,j) = 0
          end if
c
          if (abs(N_sum(2,j)).ge.abs(s(2,j)*ttol)) then
            s(2,j) = s(2,j) + N_sum(2,j)
            tflag(2,j) = 1
            tcount(2,j) = tcount(2,j) + 1.d0
          else 
            tflag(2,j) = 0
          end if
c
          if (abs(N_sum(3,j)).ge.abs(s(3,j)*ttol)) then
            s(3,j) = s(3,j) + N_sum(3,j)
            tflag(3,j) = 1
            tcount(3,j) = tcount(3,j) + 1.d0
          else 
            tflag(3,j) = 0
          end if
c
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c     MFO_SPINORB.FOR    (ErikSoft   28 June 2003)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c------------------------------------------------------------------------------
c
      subroutine mfo_spinorb (jcen,nbod,nbig,m,x,rcrit,a,as,stat,c,moi)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 jcen(3),m(nbod),x(3,nbod),a(3,nbod),as(3,nbod),rcrit(nbod)
      real*8 c(3,3,nbod),moi(3,nbod)
c
c Local
      integer i,j,k
      real*8 bigx(3),ir(3),grh(3),bigr_2(nbod),bigr_5(nbod),bigr_7(nbod)
		real*8 tmp1,tmp2
      real*8 dx,dy,dz
c
c------------------------------------------------------------------------------
...
c
c Calculate acceleration and change in spin vector due to the central body
c First convert r to Body Frame using C transpose
c   added small-body spin-tracking JWB
		do i = 2, nbod
c        spin-track only for body 2 & smalls
			if (i .le. nbig) then
				if (i .ne. 2) then
					goto 101
				endif
...
c	write (*,*) 'calculating central body torques for i=',i,'/',nbod
        bigx(1) = c(1,1,i)*x(1,2) + c(2,1,i)*x(2,2) + c(3,1,i)*x(3,2)
    	bigx(2) = c(1,2,i)*x(1,2) + c(2,2,i)*x(2,2) + c(3,2,i)*x(3,2)
    	bigx(3) = c(1,3,i)*x(1,2) + c(2,3,i)*x(2,2) + c(3,3,i)*x(3,2)
    	bigr_2(i) = 1.d0 / 
     %	            (bigx(1)*bigx(1)+bigx(2)*bigx(2)+bigx(3)*bigx(3))
    	bigr_5(i) = bigr_2(i) * bigr_2(i) * sqrt(bigr_2(i))
    	bigr_7(i) = bigr_5(i) * bigr_2(i)
			
c			if (i.eq.2) then
c				write(*,*) 'c matrix for planet ', i, ': c(1,1)=', c(1,1,i)
c				write(*,*) '      c(1,2)=', c(1,2,i), ', and c(2,3)=', c(2,3,i)
c	  		endif
c			if (i.ge.10) then
c				write(*,*) 'c matrix for planet ', i, ': c(1,1)=', c(1,1,i)
c				write(*,*) '      c(1,2)=', c(1,2,i), ', and c(2,3)=', c(2,3,i)
...
c
c Calculate IR, where I is (diagonalized) moment of inertia tensor
      	ir(1) = moi(1,i) * bigx(1)
      	ir(2) = moi(2,i) * bigx(2)
      	ir(3) = moi(3,i) * bigx(3)
c			write (*,*) ' moi(3,',i,') = ', moi(3,i)
c
c Calculate gradient of H_int
      	tmp1 = -1.5d0 * m(1) * bigr_5(i) * (moi(1,i) + moi(2,i) + 
     % 	       moi(3,i)) + 7.5d0 * m(1) * bigr_7(i) * (bigx(1)*ir(1) +
     %         bigx(2)*ir(2) + bigx(3)*ir(3))
      	tmp2 = -3.d0 * m(1) * bigr_5(i)
      	grh(1) = -(tmp1 * bigx(1)  +  tmp2 * ir(1))
      	grh(2) = -(tmp1 * bigx(2)  +  tmp2 * ir(2))
      	grh(3) = -(tmp1 * bigx(3)  +  tmp2 * ir(3))
c
c Calculate accelerations and rates of change of spin vector

        if (i.eq.2) then
            a(1,i) = a(1,i) + c(1,1,i)*grh(1) + c(1,2,i)*grh(2) + 
     %               c(1,3,i)*grh(3)
            a(2,i) = a(2,i) + c(2,1,i)*grh(1) + c(2,2,i)*grh(2) +
     %               c(2,3,i)*grh(3)
            a(3,i) = a(3,i) + c(3,1,i)*grh(1) + c(3,2,i)*grh(2) + 
     %                c(3,3,i)*grh(3)
        endif
            
        as(1,i) = (bigx(2) * grh(3)  -  bigx(3) * grh(2))
        as(2,i) = (bigx(3) * grh(1)  -  bigx(1) * grh(3))
        as(3,i) = (bigx(1) * grh(2)  -  bigx(2) * grh(1))

c			if (i.eq.2) then
c				write(*,*) 'as from Sun only for planet ', i, ': as(1)=', as(1,i)
c				write(*,*) '      as(2)=', as(2,i), ', and as(3)=', as(3,i)
c				write(*,*) '      as mag=', sqrt(as(1,i)*as(1,i)+as(2,i)*as(2,i)
c     %	      +as(3,i)*as(3,i))
c	  		endif
c			if (i.eq.10) then
c				write(*,*) 'as from Sun only for planet ', i, ': as(1)=', as(1,i)
...
c Calculate acceleration and change in spin vector due to other bodies
      	do j = 3, nbig
      		  dx = x(1,2) - x(1,j)
        		dy = x(2,2) - x(2,j)
        		dz = x(3,2) - x(3,j)
        		bigx(1) = c(1,1,i)*dx + c(2,1,i)*dy + c(3,1,i)*dz
        		bigx(2) = c(1,2,i)*dx + c(2,2,i)*dy + c(3,2,i)*dz
        		bigx(3) = c(1,3,i)*dx + c(2,3,i)*dy + c(3,3,i)*dz
        		bigr_2(i) = 1.d0 / (bigx(1)*bigx(1) + bigx(2)*bigx(2)
     %                      + bigx(3)*bigx(3))
        		bigr_5(i) = bigr_2(i) * bigr_2(i) * sqrt(bigr_2(i))
        		bigr_7(i) = bigr_5(i) * bigr_2(i)
...
c Calculate gradient of H_int
        		tmp1 = -1.5d0 * m(j) * bigr_5(i) * (moi(1,i) + moi(2,i) 
     %   		       + moi(3,i)) + 7.5d0 * m(j) * bigr_7(i) 
     %                 * (bigx(1)*ir(1) + bigx(2)*ir(2) + bigx(3)*ir(3))
        		tmp2 = -3.d0 * m(j) * bigr_5(i)
        		grh(1) = -(tmp1 * bigx(1)  +  tmp2 * ir(1))
        		grh(2) = -(tmp1 * bigx(2)  +  tmp2 * ir(2))
        		grh(3) = -(tmp1 * bigx(3)  +  tmp2 * ir(3))
...
c Calculate accelerations and rates of change of spin vector
				if (i.eq.2) then
		        	a(1,i) = a(1,i) + c(1,1,i)*grh(1) + 
     %                       c(1,2,i)*grh(2) + c(1,3,i)*grh(3)
		  	     	a(2,i) = a(2,i) + c(2,1,i)*grh(1) + 
     %                       c(2,2,i)*grh(2) + c(2,3,i)*grh(3)
                    a(3,i) = a(3,i) + c(3,1,i)*grh(1) + 
     %                        c(3,2,i)*grh(2) + c(3,3,i)*grh(3)
				endif

        		as(1,i) = as(1,i) + (bigx(2) * grh(3) - bigx(3) * grh(2))
        		as(2,i) = as(2,i) + (bigx(3) * grh(1) - bigx(1) * grh(3))
        		as(3,i) = as(3,i) + (bigx(1) * grh(2) - bigx(2) * grh(1))
      	end do
 101 	end do
c
c------------------------------------------------------------------------------
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MFO_SPIN.FOR    (ErikSoft   27 June 2003)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
c
c Updates the spin angular momentum of an unperturbed axisymmetric rigid
c body as seen in a frame rotating with the body. H is the timestep, and
c MOI are the principal moments of inertia.
c
      subroutine mfo_spin (nbod,bigs,c,h,moi,nbig)
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig
      real*8 bigs(3,nbod),c(3,3,nbod),h,moi(3,nbod)
c
c Local
      integer i,j,k
      real*8 alpha,sal,cal,temp,beta,betah,comega(3,3),omega,com,som
      real*8 sn(3),smag,cz(3,3),ctemp(3,3),c_1(3,3),c0(3,3)
c
c------------------------------------------------------------------------------
c
		do k = 2, nbod

c        do this just for body 2 and the smalls     
			if (k .le. nbig) then
				if (k .ne. 2) then
					goto 999
				endif
...
c			if (k.eq.2) then
c				write(*,*) 'moi for body ', k, ': moi(1)=', moi(1,k)
c				write(*,*) '      moi(2)=', moi(2,k), ', and moi(3)=', moi(3,k)
c				write(*,*) '      moi mag=', sqrt(moi(1,k)*moi(1,k)+moi(2,k)*moi(2,k)
c     %	      +moi(3,k)*moi(3,k))
c	  		endif
c			if (k.eq.10) then
c				write(*,*) 'moi for body ', k, ': moi(1)=', moi(1,k)
...
c Store original version of C array	
	      do i = 1, 3
  		      c0(1,i) = c(1,i,k)
  	  	 	   c0(2,i) = c(2,i,k)
  	  	   	c0(3,i) = c(3,i,k)
...
c
	      alpha = (1.d0/moi(3,k) - 1.d0/moi(1,k)) * bigs(3,k) * h
	      call mco_sine (alpha,sal,cal)
c
c Set up C_omega matrix
	      smag = sqrt(bigs(1,k)*bigs(1,k) + bigs(2,k)*bigs(2,k)
     % 	         + bigs(3,k)*bigs(3,k))
	      omega = smag / moi(1,k) * h
	      call mco_sine (omega,som,com)
c	      WRITE (*,*) 'Omega, sin, cos: ',omega,som,com
	      temp = 1.d0 / smag
	      sn(1) = bigs(1,k) * temp
	      sn(2) = bigs(2,k) * temp
	      sn(3) = bigs(3,k) * temp
	      temp = 1.d0 - com
c
	      comega(1,1) =  com        +  temp*sn(1)*sn(1)
	      comega(1,2) =  som*sn(3)  +  temp*sn(1)*sn(2)
	      comega(1,3) = -som*sn(2)  +  temp*sn(1)*sn(3)
	      comega(2,1) = -som*sn(3)  +  temp*sn(1)*sn(2)
	      comega(2,2) =  com        +  temp*sn(2)*sn(2)
	      comega(2,3) =  som*sn(1)  +  temp*sn(2)*sn(3)
	      comega(3,1) =  som*sn(2)  +  temp*sn(1)*sn(3)
	      comega(3,2) = -som*sn(1)  +  temp*sn(2)*sn(3)
	      comega(3,3) =  com        +  temp*sn(3)*sn(3)
c
c        WRITE (*,*) 'C_omega matrix'
c        WRITE (*,112) comega(1,1),comega(1,2),comega(1,3)
c        WRITE (*,112) comega(2,1),comega(2,2),comega(2,3)
c        WRITE (*,112) comega(3,1),comega(3,2),comega(3,3)
c 112    FORMAT ('    (',f9.6,1x,f9.6,1x,f9.6,')')
c
c Set up C_z matrix
	      cz(1,1) =  cal
	      cz(1,2) =  sal
	      ...
	      cz(3,3) = 1.d0
c
c Calculate product C_z * C_omega
	      do i = 1, 3
	        do j = 1, 3
	          ctemp(i,j) = cz(i,1)*comega(1,j) + cz(i,2)*comega(2,j)
     % 	              + cz(i,3)*comega(3,j)
	        end do
	      end do
c
c Calculate inverse of this product
	      call invert(ctemp,c_1)
c
c Update the vector that transforms body vectors to inertial space vectors
	      do i = 1, 3
	        do j = 1, 3
	          c(i,j,k) = c0(i,1)*c_1(1,j) + c0(i,2)*c_1(2,j)
     %           + c0(i,3)*c_1(3,j)
	        end do
	      end do
c
c Update the spin angular momentum (Z component is unchanged)
	      temp =       bigs(1,k) * cal  +  bigs(2,k) * sal
	      bigs(2,k) = -bigs(1,k) * sal  +  bigs(2,k) * cal
	      bigs(1,k) = temp
c
c------------------------------------------------------------------------------
c
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      INVERT.FOR    (ErikSoft   27 June 2003)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
c
c Given a 3-3 matrix C, the subroutine provides the inverse matrix C_1.
c The original matrix is not altered.
c
      subroutine invert (c,c_1)
      implicit none
c
c Input/Output
      real*8 c(3,3),c_1(3,3)
c
c Local
      integer i,j
      real*8 cc(3,3),tmp1,tmp2,tmp3
c
c------------------------------------------------------------------------------
c Store a copy of the matrix
      do i = 1, 3
        do j = 1, 3
          cc(i,j)  = c(i,j)
          c_1(i,j) = 0.d0
...
c Invert the lower triangle of matrix C
      ...
      tmp2 = cc(3,1) / cc(1,1)
      do j = 1, 3
         cc(2,j) =  cc(2,j)  -  tmp1 *  cc(1,j)
        c_1(2,j) = c_1(2,j)  -  tmp1 * c_1(1,j)
         cc(3,j) =  cc(3,j)  -  tmp2 *  cc(1,j)
        c_1(3,j) = c_1(3,j)  -  tmp2 * c_1(1,j)
      end do
      tmp1 = cc(3,2) / cc(2,2)
      do j = 1, 3
         cc(3,j) =  cc(3,j)  -  tmp1 *  cc(2,j)
        c_1(3,j) = c_1(3,j)  -  tmp1 * c_1(2,j)
      end do
c
c Invert the diagonal elements
      tmp1 = 1.d0 / cc(1,1)
      tmp2 = 1.d0 / cc(2,2)
      tmp3 = 1.d0 / cc(3,3)
      do j = 1, 3
         cc(1,j) =  cc(1,j) * tmp1
        c_1(1,j) = c_1(1,j) * tmp1
         cc(2,j) =  cc(2,j) * tmp2
        c_1(2,j) = c_1(2,j) * tmp2
         cc(3,j) =  cc(3,j) * tmp3
        c_1(3,j) = c_1(3,j) * tmp3
      end do
c
c Invert the upper triangle of matrix C
      tmp1 = cc(1,3)
      tmp2 = cc(2,3)
      do j = 1, 3
         cc(1,j) =  cc(1,j)  -  tmp1 *  cc(3,j)
        c_1(1,j) = c_1(1,j)  -  tmp1 * c_1(3,j)
         cc(2,j) =  cc(2,j)  -  tmp2 *  cc(3,j)
        c_1(2,j) = c_1(2,j)  -  tmp2 * c_1(3,j)
      end do
      tmp1 = cc(1,2)
      do j = 1, 3
         cc(1,j) =  cc(1,j)  -  tmp1 *  cc(2,j)
        c_1(1,j) = c_1(1,j)  -  tmp1 * c_1(2,j)
      end do
c
c------------------------------------------------------------------------------
c
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Steven M. Kreyche (19 May 2020)
c
c This subroutine computes tidal obliquity torques from the central body onto 
c body 2 and the small bodies from Bolmont et al. (2015). These torques act to
c despin the planets as well as reduce their obliquities.
c
c Input coordinates must be in heliocentric (central) coordinates!
c
c------------------------------------------------------------------------------
c
      subroutine obl_tides (nbod,nbig,m,xx,xy,xz,vx,vy,vz,s,C_MR2,R_p,
     %                      k_2p,t_lag,N_t,h0)
c
c nbod = total number of bodies (big and small) [unitless]
c nbig = number of big bodies [unitless]
c m = vector containing body masses [solar masses * K2]
c xx = body 2 x position component [AU]
c xy = body 2 y position component [AU]
c xz = body 2 z position component [AU]
c vx = body 2 x velocity component [AU / day]
c vy = body 2 y velocity component [AU / day]
c vz = body 2 z velocity component [AU / day]
c s = matrix containing spin angular momentum components [solar masses * AU^2 / day * K2]
c C_MR2 = vector containing body momement of inertia coefficient [unitless]
c R_p = vector containing body equatorial radii [AU]
c k_2p = vector containing body potential love numbers of degree 2 [unitless]
c t_lag = vector containing body tidal time lag values [days]
c N_t = matrix containing body tidal obliquity torque components (x,y,z) [solar masses * AU^2 / day^2 * K2]
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig
      real*8 m(nbod),xx,xy,xz,vx,vy,vz,s(3,nbod),R_p(nbod),k_2p(nbod)
      real*8 t_lag(nbod),N_t(3,nbod),C_MR2(nbod),h0
c
c Local
      integer j
      real*8 r_mag,r_dot_omega,P_to,m_factor,I_p,omegax,omegay,omegaz
      real*8 R_p2
c
c------------------------------------------------------------------------------
c
c Gathering some unverisal parts
        r_mag = sqrt(xx*xx + xy*xy + xz*xz)
        m_factor= -(K2 * h0 * m(1)) / (m(1) + m(2))
c
c Perform for body 2 and small bodies (ghost planets)
      do j = 2, nbod
        if (j.le.nbig) then
          if (j.ne.2) then
            goto 10
...
c Gathering some additional parts
        R_p2 = R_p(j)*R_p(j)
        I_p = C_MR2(j)*m(2)/K2*R_p2
        omegax = s(1,j) / K2 / I_p
        omegay = s(2,j) / K2 / I_p
        omegaz = s(3,j) / K2 / I_p
        r_dot_omega = xx*omegax + xy*omegay + xz*omegaz
        P_to = (3.d0*m(1)*m(1)*R_p2*R_p2*R_p(j)*k_2p(j)*t_lag(j)) / 
     %         (K2*r_mag**7.d0)
c
c Computing the torque components
        N_t(1,j) = P_to * (r_mag*omegax - r_dot_omega*xx/r_mag - 
     %             (xy*vz - xz*vy)/r_mag )
        N_t(2,j) = P_to * (r_mag*omegay - r_dot_omega*xy/r_mag -
     %             (xz*vx - xx*vz)/r_mag )   
        N_t(3,j) = P_to * (r_mag*omegaz - r_dot_omega*xz/r_mag -
     %             (xx*vy - xy*vx)/r_mag ) 
c
c Final values adjusted for heliocentric coordinates and multiply by K2
        N_t(1,j) = m_factor * N_t(1,j)
        N_t(2,j) = m_factor * N_t(2,j)
        N_t(3,j) = m_factor * N_t(3,j)
c        
10    end do
c
c------------------------------------------------------------------------------
...
c
c N.B. Input/output must be in heliocentric (central) coordinates!
c ===
c
c-----------------------------------------------------------------------------
c
c Author: Steven M. Kreyche (21 January 2020)
c
c This subroutine incorporates general relativistic force corrections 
c following Bolmont et al. (2015). Returns GR acceleration components (x,y,z) 
c within the matrix a_GR for each big body due to their interactions with the
c central body. 
c
c------------------------------------------------------------------------------
c
      subroutine mfo_user (nbod,nbig,hby2,m,x,v,v_GR)
c
c nbod = current number of bodies (INCLUDING the central object)
c nbig = big bodies (bodies that perturb others)
c hby2 = half of the timestep [day]
c m = masses [Solar masses * K2]
c x = position coordinates (x,y,z) [AU]
c v = velocities (vx,vy,vz) [AU/day]
c v_GR = velocities (vx,vy,vz) [AU/day]
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig
      real*8 m(nbod),x(3,nbod),v(3,nbod),v_GR(3,nbig),hby2
c
c Local
      integer j
      real*8 sum_cor(3),m_ratio,GR_param,r2,r_mag,v2,v_mag,v_rad
      real*8 FGR_rad,FGR_ort,a_GR(3,nbig)
c
c------------------------------------------------------------------------------
c
c Initializing sum components of corrective acceleration to 0
      sum_cor(1) = 0.d0
      sum_cor(2) = 0.d0
      sum_cor(3) = 0.d0
c     
c Computing for big bodies
      do j = 2, nbig
...
c     
c Orthoradial component of general relativisitic force
        FGR_ort = 2.d0*(2.d0-GR_param) * (m(1)+m(j)) /
     %            (r2*lightspeed*lightspeed) * v_rad*v_mag 
c
c Components of relativistic acceleration (x,y,z)
        a_GR(1,j) = FGR_rad*x(1,j)/r_mag + FGR_ort*v(1,j)/v_mag
        a_GR(2,j) = FGR_rad*x(2,j)/r_mag + FGR_ort*v(2,j)/v_mag
        a_GR(3,j) = FGR_rad*x(3,j)/r_mag + FGR_ort*v(3,j)/v_mag
c
c Adding contributions to corrective acceleration component sums
        sum_cor(1) = sum_cor(1) + m_ratio*a_GR(1,j)
        sum_cor(2) = sum_cor(2) + m_ratio*a_GR(2,j)
        sum_cor(3) = sum_cor(3) + m_ratio*a_GR(3,j)    
      end do
c      
c Obtain total relativistic accelerations by adding corrections, then find velocities
      do j = 2, nbig
        v_GR(1,j) = hby2 * (a_GR(1,j) + sum_cor(1))
        v_GR(2,j) = hby2 * (a_GR(2,j) + sum_cor(2))
        v_GR(3,j) = hby2 * (a_GR(3,j) + sum_cor(3)) 
      end do
c
c------------------------------------------------------------------------------
c
...
c Input/Output
      ...
      real*8 C_MR2(nbod),k_2p(nbod),t_lag(nbod),ttol,tcount(3,nbod)
      character*8 id(nbod)
      character*80 outfile(3),dumpfile(4),mem(NMESS)
c
c Local
...
c Output data for all bodies
     ...
     %    stat,id,opt,opflag,algor,outfile(1),C_MR2,tcount)
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MAL_HCON.FOR    (ErikSoft   30 June 2003)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c
     ...
     %  onestep,coord,bcoord,C_MR2,k_2p,t_lag,ttol,tcount)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 obl_max(nbod),obl_min(nbod),tcount(3,nbod)
      character*8 id(nbod)
      character*80 outfile(3),dumpfile(4),mem(NMESS)
c
c Local
...
c     %  m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,opt,outfile(2),1)
c
c Set up time of next output, times of previous dump, log and periodic effect
      if (opflag.eq.-1) then
        tout = tstart
...
c Convert to internal coordinates and velocities
      ...
      call mxx_obl (jcen,nbod,nbig,m,xh,vh,s,rho,obl_max,obl_min,1)
...
c (JWB) execute timestep
     ...
     %  mem,lmem,C_MR2,rho,k_2p,t_lag,ttol,tcount)
      time = time + h0
c Modification for Spin version of Mercury: ie spin will change every step
      if (opflag.ge.0) opflag = 1
		
c (JWB) set position of ghost planets to that of object 2
		do i = nbig+1, nbod
			x(1,i) = x(1,2)
			x(2,i) = x(2,2)
			x(3,i) = x(3,2)
			
c			write (*,*) 's(1,2) = ', s(1,2), '; s(1,E+180) = ', s(1,nbig+1)
c			write (*,*) 's(2,2) = ', s(2,2), '; s(2,E+180) = ', s(2,nbig+1)
c			write (*,*) 's(3,2) = ', s(3,2), '; s(3,E+180) = ', s(3,nbig+1)
		end do
...
c        call mio_ce (time,tstart,rcen,rmax,nbod,nbig,m,stat,id,nclo,
c     %    iclo,jclo,opt,stopflag,tclo,dclo,ixvclo,jxvclo,mem,lmem,
c     %    outfile,nstored,itmp)
...
c      if (colflag.ne.0) then
c
c Reindex the surviving objects
c        call bcoord (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
c        call mxx_elim (nbod,nbig,m,xh,vh,s,rho,rceh,rcrit,ngf,stat,
c     %    id,mem,lmem,outfile(3),itmp)
c
c Reset flags, and calculate new Hill radii and physical radii
c        dtflag = 1
c        if (opflag.ge.0) opflag = 1
c        call mce_init (tstart,algor,h0,jcen,rcen,rmax,cefac,nbod,nbig,
c     %    m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,opt,outfile(2),1)
c        call coord (time,jcen,nbod,nbig,h0,m,xh,vh,x,v,ngf,ngflag,opt)
c      end if
c
c------------------------------------------------------------------------------
c
...
c      if (algor.eq.1) then
c        call mco_iden(time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
c      else
c        call bcoord (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
c      end if
c      itmp = 2
c      if (algor.eq.11.or.algor.eq.12) itmp = 3
c      call mce_cent (time,h0,rcen,jcen,itmp,nbod,nbig,m,xh0,vh0,xh,vh,
c     %  nhit,jhit,thit,dhit,algor,ngf,ngflag)
c
c If something hit the central body, restore the coords prior to this step
c      if (nhit.gt.0) then
c        call mco_iden (time,jcen,nbod,nbig,h0,m,xh0,vh0,xh,vh,ngf,
c     %    ngflag,opt)
c        time = time - h0
c
...
c          i = 1
c          j = jhit(k)
c          call mce_coll (thit(k),tstart,en(3),jcen,i,j,nbod,nbig,m,xh,
c     %      vh,s,rphys,stat,id,opt,mem,lmem,outfile(3))
c        end do
c
c Remove lost objects, reset flags and recompute Hill and physical radii
c        call mxx_elim (nbod,nbig,m,xh,vh,s,rho,rceh,rcrit,ngf,stat,
...
c        dtflag = 1
c        call mce_init (tstart,algor,h0,jcen,rcen,rmax,cefac,nbod,nbig,
c     %    m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,opt,outfile(2),0)
c        if (algor.eq.1) then
c          call mco_iden (time,jcen,nbod,nbig,h0,m,xh,vh,x,v,ngf,ngflag,
c     %      opt)
c        else
c          call coord (time,jcen,nbod,nbig,h0,m,xh,vh,x,v,ngf,ngflag,opt)
c        end if
...
c Local
      ...
      real*8 ha(2),hb(2),rt10,angf(3,NMAX),a_GR(3,nbod)
c
c------------------------------------------------------------------------------
c
      rt10 = sqrt(10.d0)
...
c
        do j = 2, nbod
          vh(1,j) = vh(1,j)  -  hb(k) * (angf(1,j) + a_GR(1,j) + a(1,j))
          vh(2,j) = vh(2,j)  -  hb(k) * (angf(2,j) + a_GR(2,j) + a(2,j))
          vh(3,j) = vh(3,j)  -  hb(k) * (angf(3,j) + a_GR(3,j) + a(3,j))
        end do
c
c Advance Keplerian Hamiltonian (Jacobi/helio coords for Big/Small bodies)
        call mco_h2j(time,jcen,nbig,nbig,h,m,xh,vh,xj,vj,ngf,ngflag,opt)
...
c
        do j = 2, nbod
          v(1,j) = v(1,j)  +  hb(k) * (angf(1,j) + a_GR(1,j) + a(1,j))
          v(2,j) = v(2,j)  +  hb(k) * (angf(2,j) + a_GR(2,j) + a(2,j))
          v(3,j) = v(3,j)  +  hb(k) * (angf(3,j) + a_GR(3,j) + a(3,j))
        end do
c
c Advance Keplerian Hamiltonian (Jacobi/helio coords for Big/Small bodies)
        call mco_h2j (time,jcen,nbig,nbig,h,m,x,v,xj,vj,ngf,ngflag,opt)
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MCO_X2IN.FOR    (ErikSoft  23 January 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Billy Quarles
c
c Calculates inclination and ascending node given relative coordinates and
c velocities, and GM = G times the sum of the masses.
c
c The elements are: i = inclination
c                   n = longitude of ascending node
c
c------------------------------------------------------------------------------
c
      subroutine mco_x2in (gm,x,y,z,u,v,w,i,n)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      real*8 gm,i,n,x,y,z,u,v,w
c
c Local
      real*8 hx,hy,hz,h2,h,ci
c
c------------------------------------------------------------------------------
c
      hx = y * w  -  z * v
...
c Local
      ...
      real*8 angf(3,NMAX),a_GR(3,nbod)
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
...
c Advance interaction Hamiltonian for H/2
      do j = 2, nbod
        v(1,j) = v(1,j)  +  hby2 * (angf(1,j) + a_GR(1,j) + a(1,j))
        v(2,j) = v(2,j)  +  hby2 * (angf(2,j) + a_GR(2,j) + a(2,j))
        v(3,j) = v(3,j)  +  hby2 * (angf(3,j) + a_GR(3,j) + a(3,j))
      end do
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
c
      do i = i0, nbig
        do j = i + 1, nbig
c                       ^  changed to nbig from nbod JWB -- smalls follow body 2
          dx = x(1,j) - x(1,i)
          dy = x(2,j) - x(2,i)
          dz = x(3,j) - x(3,i)
          s2 = dx * dx  +  dy * dy  +  dz * dz
...
c Local
      ...
      real*8 tmp0,t1,rfac,fr,fv,theta,phi,vtheta,vphi,temp
      character*80 c(200)
      character*38 fstop
...
c JWB added i and j to dmp outputs for obliquity-tracking restart capability
...
c Important parameters
        ...
        else if (algor.eq.9) then
          write (33,*) mem(159)(1:lmem(159)),'SPIN'
...
c Integration options
        ...
        if (opt(6).eq.1) then
          write (33,'(2a)') mem(172)(1:lmem(172)),mem(6)(1:lmem(6))
        else
          write (33,'(2a)') mem(172)(1:lmem(172)),mem(5)(1:lmem(5))
        end if
        if (opt(7).eq.1) then
...
c
c Local
      integer j,k,itmp,jtmp,informat,lim(2,15),nsub,year,month,lineno
...
c
     ...
     %   'xxx','SPI','Spi','spi','SPN','spn','HYB','Hyb','hyb','HY ',
...
c Read values of mass, close-encounter limit, density etc.
        do k = 3, nsub, 2
          ...
          if (c80(1:1).eq.'m'.or.c80(1:1).eq.'M') then
            m(nbod) = temp * K2
c I contains scaled moment of inertia (i.e. in units of MR^2)
          else if (c80(1:1).eq.'i'.or.c80(1:1).eq.'I') then
            C_MR2(nbod) = temp
c J contains the J2 coefficient for this body
c          else if (c80(1:1).eq.'j'.or.c80(1:1).eq.'J') then
c            J_2(nbod) = temp
c k_2p contains the tidal potential love number degree 2 coefficient for this body
          else if (c80(1:1).eq.'k'.or.c80(1:1).eq.'K') then
            k_2p(nbod) = temp
c t_lag contains tidal time lag value for this body
          else if (c80(1:1).eq.'t'.or.c80(1:1).eq.'T') then
            t_lag(nbod) = temp/86400.d0
...
c Calculate moments of inertia 
c			if (j.eq.1) then
c        		temp = m(nbod) * (.75d0*m(nbod) / (PI*rho(nbod)))**(2.d0/3.d0)
c			endif
c			if (j.eq.2) then
c        		temp = m(2) * (.75d0*m(2) / (PI*rho(2)))**(2.d0/3.d0)
c			endif
c        moi(1,nbod) = C_MR2(nbod) - J_2(nbod)
c        moi(3,nbod) = C_MR2(nbod) * temp
c        moi(1,nbod) = moi(1,nbod) * temp
c        moi(2,nbod) = moi(1,nbod)
        goto 130
 140    close (11)
      end do
c
c Set non-gravitational-forces flag, NGFLAG
	...
	write (*,*) 'nbod determined to be ',nbod
c
c------------------------------------------------------------------------------
c
...
c
c Check if obliquity tide (phase lag) routine is being used with wrong algorithm -- SMK
      if (opt(6).eq.1.and.(algor.eq.1.or.algor.eq.2.or.algor.eq.3.or.
     %                     algor.eq.4.or.algor.eq.10.or.algor.eq.11.or.
     %                     algor.eq.12)) call mio_err
     %  (23,mem(81),lmem(81),mem(109),lmem(109),' ',1,mem(85),lmem(85))
c
c Check if obliquity tide (time lag) routine is being used with wrong algorithm -- SMK
      if (opt(7).eq.1.and.(algor.eq.1.or.algor.eq.2.or.algor.eq.3.or.
     %                     algor.eq.4.or.algor.eq.10.or.algor.eq.11.or.
     %                     algor.eq.12)) call mio_err
     %  (23,mem(81),lmem(81),mem(109),lmem(109),' ',1,mem(85),lmem(85))
c
...
c
     ...
     %  stat,id,opt,opflag,algor,outfile,C_MR2,tcount)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 s(3,nbod),rho(nbod),C_MR2(nbod),k_2p(nbod),t_lag(nbod)
      real*8 tcount(3,nbod)
      character*80 outfile
      character*8 id(nbod)
c
c Local
...
c added C_MR2 for element6 computation and output -- SMK
        do k = 2, nbod
          ...
          c(k)(60:67) = mio_fl2c (tcount(1,k))
          c(k)(68:75) = mio_fl2c (tcount(2,k))
          c(k)(76:83) = mio_fl2c (tcount(3,k))
        end do
c
c Write compressed output to file
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c     MXX_OBL.FOR    (ErikSoft   2 March 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Billy Quarles
c
c Calculates the minimum and maximum obliquity of ghost particles.
c
c N.B. All coordinates and velocities must be heliocentric!!
c ===
c
c------------------------------------------------------------------------------
c
      subroutine mxx_obl (jcen,nbod,nbig,m,x,v,s,rho,obl_max,obl_min,
     %                    init)
c
      implicit none
      include 'mercury.inc'
...
c Input/Output
      integer nbod,nbig
      real*8 jcen(3),m(nbod),x(3,nbod),v(3,nbod),s(3,nbod),rho(nbod)
      real*8 obl_max(nbod),obl_min(nbod)
c
c Local
      integer j,itmp(8),iflag,init
      real*8 temp,is(nbod),ns(nbod),rhocgs
      real*8 a,e,inc,g,nl,l,obl

      rhocgs = AU * AU * AU * K2 / MSUN

      call mco_x2in (m(1) + m(2),x(1,2),x(2,2),x(3,2),v(1,2),v(2,2),
     %               v(3,2),inc,nl)
...
c Calculate longitude & inclination of spin vector
     ...
     %        + sin(inc)*sin(is(j))*cos(ns(j) - nl))
         ...
         if (init.eq.1) then
           ...
           if (obl.gt.obl_max(j)) then
              obl_max(j) = obl
           end if
           if (obl.lt.obl_min(j)) then
              obl_min(j) = obl
           end if
         end if
	end do
